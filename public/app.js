document.addEventListener('DOMContentLoaded', function () {
  ...
  function matches(card) {
    ...
    const categoryOK = state.filter === 'all' || category === state.filter ||
  ...
  function sortCards(list) {
    return list.slice().sort(function (a, b) {
      ...
      const at = (a.dataset.title || '').toLowerCase(), bt = (b.dataset.title || '').toLowerCase();
  ...
  function renderPagination(totalPages) {
    ...
    pagination.appendChild(makePageButton('‹', state.page - 1, false, state.page === 1));
    ...
    if (totalPages <= 7) {
      ...
      pages.push(1, 'dots', state.page - 1, state.page, state.page + 1, 'dots', totalPages);
    }
    ...
    pages.forEach(function (item) {
      if (item === 'dots') {
        ...
        pagination.appendChild(makePageButton(String(item), item, item === state.page, false));
      }
    });

    pagination.appendChild(makePageButton('›', state.page + 1, false, state.page === totalPages));
  }
  ...
  function render() {
    ...
    const visible = matching.slice(start, start + state.perPage);
    ...
    visible.slice().reverse().forEach(function (card) { grid.prepend(card); });

    if (count) {
      const shown = Math.min(state.perPage, Math.max(0, total - start));
      count.textContent = total ? ('Mostrando ' + shown + ' de ' + total + ' writeups') : 'Mostrando 0 writeups';
    }
  ...
  if (search) search.addEventListener('input', function () {
    ...
    state.page = 1;
    if (clear) clear.style.visibility = state.query ? 'visible' : 'hidden';
    render();
  });
